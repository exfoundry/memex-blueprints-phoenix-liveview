...
Every LiveView needs a page title, a path for nav active state, and breadcrumb context. Without a standard these scatter across templates and layouts. The [`page_meta`](https://github.com/exfoundry/page_meta) library centralises this into a single struct with compile-time behaviour binding — forgetting `page_meta/2` is a compile warning, not a runtime crash.

## Decision

The struct lives in `PlatformWeb.PageMeta`, defined explicitly via `defstruct` (no macro hides the fields). It implements `PageMeta.Site` for site-wide concerns (`endpoint_url/0`, `lang_path/2`) and delegates `breadcrumbs/1` and `active?/2,3` to the package. The package binds to it at compile time:

```elixir
config :page_meta, app_module: PlatformWeb.PageMeta
```

Every LiveView implements `page_meta/2` (`@behaviour PageMeta.LiveView`). The `live_view` macro in `PlatformWeb` adds the behaviour and imports `assign_page_meta/1` automatically.
...
The package is named `PageMeta`, the project module is also `PageMeta` (aliased from `PlatformWeb.PageMeta` in `html_helpers`). In Layouts (where `root.html.heex` lives), the project alias shadows the package's top-level. To call package components from Layouts use a side-by-side alias:

```elixir
alias PlatformWeb.PageMeta                # project struct
alias Elixir.PageMeta.SEO, as: PageMetaSEO  # package SEO component
```

Other modules (e.g. `PlatformWeb.Components.Breadcrumb`) that do not `use PlatformWeb, :html` see top-level `PageMeta` directly and can use `PageMeta.Breadcrumb.list` without disambiguation.
...
- Don't render breadcrumbs by hand. Use `<Breadcrumb.default page_meta={@page_meta} />` (project wrapper) which delegates to `<PageMeta.Breadcrumb.list>` with project styling and home-icon-on-root behaviour
- Don't render SEO meta tags by hand. Use `<PageMetaSEO.meta_tags page_meta={@page_meta} />` in `root.html.heex`

## Active-state helpers

Two project helpers in `PlatformWeb.Layouts` bridge to `PageMeta.active?/3`, handling locale-prefix stripping:
...
Both accept paths *without* locale prefix; the helpers prepend it before calling `PageMeta.active?/3`. Slash-boundary matching is inherited from the package (`/locations` matches `/locations/123` but not `/location-foo`).
...
| Module | Purpose |
|---|---|
| `PageMeta` | Top-level helpers: `breadcrumbs/1` (delegate), `active?/2,3` |
| `PageMeta.Breadcrumb` | Struct, `build/1`, slot-based `<.list>` component |
| `PageMeta.SEO` | `<.meta_tags>` component |
| `PageMeta.Site` | Behaviour: `endpoint_url/0`, `lang_path/2` |
| `PageMeta.LiveView` | Behaviour: `page_meta/2`. Helper: `assign_page_meta/1` |
